...
\thispagestyle{empty}
\rhead{ \textit{ }}
\fancyhead[LO]{State-Aware Kernel Fuzzing via Caller-Callee Data-Flow Extraction}

%% Title
\title{State-Aware Kernel Fuzzing via Caller-Callee Data-Flow Extraction
\thanks{\textit{\underline{Citation}}:
\textbf{Authors. State-Aware Kernel Fuzzing via Caller-Callee Data-Flow Extraction. 2026. DOI:000000/11111.}}
}
...
implications. We present \toolname{}, an LLVM-based instrumentation framework
that extends Linux KCOV with data-flow extraction of function arguments and return
values. A compiler pass (\texttt{-fsanitize-coverage=dataflow-args,dataflow-ret})
emits lightweight callbacks capturing a structured tuple
$\langle \mathit{PC}, \mathit{arg\_idx}, \mathit{arg\_size}, \mathit{ptr},
\mathit{offsets}[] \rangle$ at function entry and
$\langle \mathit{PC}, \mathit{ret\_size}, \mathit{ptr}, \mathit{offsets}[] \rangle$
at return. Composite types are automatically decomposed via DWARF
\texttt{DICompositeType} metadata with zero source annotation. A separate kernel
device (\texttt{/sys/kernel/debug/kcov\_dataflow}) provides lock-free per-task
ring buffers with no interference to existing KCOV or syzkaller infrastructure.
We demonstrate dual utility: (1)~fuzzers gain state-aware feedback for mutation
guidance into value-dependent state transitions, and (2)~security analysts
obtain deterministic argument traces for root-cause analysis without
\texttt{printk} or kprobe overhead. A post-compilation pipeline
(\texttt{rustc}$\to$\texttt{opt}$\to$\texttt{llc}) enables Rust kernel module
instrumentation without modifying rustc---the only runtime method for capturing
Rust function arguments given that \texttt{drgn}/vmcore fails under
\texttt{-O2} DWARF elision. Evaluated on five vulnerability classes (OOB, UAF,
double-free, 10-deep chain propagation, Rust FFI) with $<$3\% overhead on
instrumented paths.
\end{abstract}

\keywords{Kernel Fuzzing \and Data-Flow Extraction \and LLVM Instrumentation \and KCOV \and Rust Kernel Modules}
...
We observe that \emph{function boundaries} are natural chokepoints where
...
\begin{itemize}[nosep]
  ...
  \item A lock-free, per-task kernel transport mechanism that operates safely in
        process, softirq, and hardirq contexts without interfering with existing
        KCOV or syzkaller infrastructure.
...
We assume a standard kernel fuzzing threat model: an unprivileged user-space
...
triggered; our tool provides the \emph{path to triggering} it and the
\emph{data for triaging} it. We assume the kernel is compiled with debug
...
compiler flags \texttt{-fsanitize-coverage=dataflow-args,dataflow-ret} and
...
\texttt{cl::opt} flags: \texttt{-sanitizer-coverage-dataflow-args} and
\texttt{-sanitizer-coverage-dataflow-ret}.
...
\texttt{u64} words: \texttt{[seq | flags | pc | arg\_idx | arg\_size |
num\_fields | field\_val$_0$ | \ldots | field\_val$_N$]}.
...
No \texttt{printk} or allocation occurs in the data path. The implementation is
safe in process, softirq, and hardirq contexts; NMI is disabled via an
\texttt{in\_nmi()} early-return check.

\subsection{Separate Device Interface}\label{sec:device}

\toolname{} exposes \texttt{/sys/kernel/debug/kcov\_dataflow} as a device
\emph{completely independent} from the legacy \texttt{/sys/kernel/debug/kcov}.
This ensures zero interference with syzkaller's existing KCOV usage.

The ioctl namespace uses magic~\texttt{'d'}:
\begin{itemize}[nosep]
  \item \texttt{KCOV\_DF\_INIT\_TRACE} (\texttt{0x80086401}): allocate buffer
...
which lacks our custom flags. We solve this with a post-compilation pipeline:
\begin{enumerate}[nosep]
  \item \texttt{rustc --emit=llvm-ir}: generate LLVM IR text with debug info
  \item \texttt{opt -passes=sancov-module -sanitizer-coverage-dataflow-args
        -sanitizer-coverage-dataflow-ret}: instrument with our pass
  \item \texttt{llc -filetype=obj}: compile to object code
  \item Standard module linking produces the instrumented \texttt{.ko}
\end{enumerate}
This works because LLVM IR is language-agnostic: our pass operates identically
...
\paragraph{Design choice: shallow capture.} We perform exactly one level of
dereference (pointer $\to$ struct fields). We do \emph{not} chase pointers
...
\texttt{SanitizerArgs.cpp} (parsing \texttt{"dataflow-args"}/\texttt{"dataflow-ret"}
...
\subsection{Kernel Backend (400~LOC)}
...
tracing for comprehensive fuzzing campaigns.
...
overhead---inlined functions lose individual argument traces but the kernel
...
We evaluate \toolname{} on linux-next 7.1.0-rc4 compiled with our custom
clang/LLVM~23 (trunk plus the dataflow pass). The kernel is configured with
\texttt{CONFIG\_KASAN=y}, \texttt{CONFIG\_KCOV=y},
\texttt{CONFIG\_KCOV\_DATAFLOW\_ARGS=y}, \texttt{CONFIG\_KCOV\_DATAFLOW\_RET=y},
\texttt{CONFIG\_RUST=y}, and \texttt{CONFIG\_DEBUG\_INFO\_DWARF5=y}. Tests run
under virtme-ng (QEMU/KVM) with 4~vCPUs and 4\,GB RAM.

Five purpose-built kernel modules exercise distinct vulnerability classes:
out-of-bounds write, use-after-free write, double-free write, 10-level deep
call chain with taint propagation, and a Rust FFI module.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Observable} & \textbf{KASAN} & \textbf{drgn} & \textbf{Ours} \\
\midrule
...
We measure overhead using a micro-benchmark that triggers 8 instrumented
...
tracing fits naturally within the accepted performance budget.
Notably, our measurements already include KASAN---the +133\% is the
\emph{additional} cost on top of an already-sanitized kernel.

\subsection{Triage Capability}

For each vulnerability case, the ENTRY/RET trace provides:
...
demonstrates that \toolname{} traces taint propagation across function
...
(6~registers), we test with 8 arguments of mixed types including a struct
...
\section{Real-World Vulnerability Discovery}\label{sec:realworld}

We applied \toolname{} to production kernel code---the Android Binder driver
(\texttt{drivers/android/binder.c} and its Rust rewrite) and Rust kernel core
subsystems (\texttt{rust/kernel/})---discovering 60 findings across 14 source
files. None are detectable by KASAN, edge coverage, or \texttt{drgn}.

\subsection{Methodology}

For each target subsystem, we: (1)~enable \texttt{KCOV\_DATAFLOW} on the
module, (2)~write a user-space program exercising edge-case inputs, (3)~capture
the \toolname{} log, (4)~analyze ENTRY/RET argument values for anomalies.
No fuzzer is used---purely deterministic log auditing.

\subsection{Critical and High Findings}

\begin{table*}[t]
\centering
\caption{Critical and High severity findings from real kernel code.}\label{tab:critical}
\small
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{Severity} & \textbf{Location} & \textbf{Finding} & \textbf{Impact} \\
\midrule
High & \texttt{rust/kernel/error.rs:441} & \texttt{from\_errno(-4096)} silently returns EINVAL & Masks real error codes; wrong code path taken \\
High & \texttt{drivers/android/binder.c} & \texttt{SET\_MAX\_THREADS} accepts any u32 & \texttt{0xffffffff} $\to$ unbounded thread creation $\to$ OOM \\
Medium & \texttt{drivers/android/binder/*.rs} & \texttt{BC\_ENTER\_LOOPER} duplicate accepted & Thread in inconsistent looper state \\
Medium & \texttt{rust/kernel/uaccess.rs} & Zero-length \texttt{UserSlice} read succeeds without validating ptr & Address never checked \\
Medium & \texttt{rust/kernel/rbtree.rs} & Duplicate key insert silently replaces & Callers expecting ``fail if exists'' get wrong behavior \\
\bottomrule
\end{tabular}
\end{table*}

\paragraph{Error Masking (High).} \texttt{Error::from\_errno(-4096)} is outside
the valid errno range $[-1, -4095]$ but silently returns \texttt{EINVAL}. The
source contains \texttt{// TODO: Make it a WARN\_ONCE}. Any C function returning
an unexpected error code gets misinterpreted by Rust callers.

\paragraph{Binder Thread Exhaustion (High).} \texttt{BINDER\_SET\_MAX\_THREADS}
stores any \texttt{u32} directly into \texttt{proc->max\_threads} without
validation. Setting \texttt{0xffffffff} allows unbounded thread creation---a
denial-of-service vector on Android where binder access is available to apps.

\subsection{Rust Kernel Core Audit (30 Findings)}

We audited \texttt{rust/kernel/\{alloc, sync, page, uaccess, fs, task, error,
rbtree, str\}.rs} with edge-case inputs:

\begin{itemize}[nosep]
  \item \textbf{KVec:} \texttt{reserve(usize::MAX)} correctly returns
        \texttt{AllocError}; \texttt{remove(0)} on empty returns \texttt{Err}
        (unlike \texttt{std::Vec} which panics); \texttt{push\_within\_capacity}
        on full vec returns the rejected value.
  \item \textbf{RBTree:} Duplicate key insert silently replaces (differs from
        \texttt{std::HashMap::try\_insert} which returns \texttt{Err}).
  \item \textbf{Arc:} \texttt{into\_unique\_or\_drop} with multiple refs
        correctly drops; refcount operations traced at \texttt{-O2}.
  \item \textbf{Page:} Cross-page boundary reads correctly rejected
        (offset+len $>$ 4096 $\to$ EINVAL).
  \item \textbf{UserSlice:} NULL, kernel-range, and unmapped addresses all
        return EFAULT. Zero-length succeeds without validation (potential issue).
\end{itemize}

\subsection{Rust Binder IPC Audit (10 Findings)}

The production Rust Binder (\texttt{drivers/android/binder/*.rs}) was exercised
via binder ioctls. Key finding: the Rust binder has a cross-process ioctl
detection (``\texttt{get\_current\_thread was called from the wrong process}'')
that does \emph{not} exist in the C binder---a security improvement only
visible through \toolname{}.

\subsection{Findings Summary}

\begin{table}[t]
\centering
\caption{Findings by severity (60 total).}\label{tab:severity}
\small
\begin{tabular}{@{}lr@{}}
\toprule
...
Medium & 8 \\
Low & 4 \\
Info (correct behavior verified) & 37 \\
Novel (Rust-specific observability) & 7 \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{\toolname{} operates at LLVM IR level.} Our pass runs on LLVM IR
...
We observe that \toolname{} enables a new form of \emph{runtime verification}
for kernel functions: verifying pre/postcondition contracts at function
boundaries without source modification, for any function the traced process
...
\texttt{-g}; without it, only scalar argument values are captured. This is not
a practical limitation for kernel fuzzing (KASAN already requires debug info).

\paragraph{Rust Pipeline.} The post-compilation pipeline is not yet integrated
into Kbuild automatically; it requires a manual wrapper invocation. We propose
adding native \texttt{-Zsanitizer-coverage=dataflow-args} support to rustc as
future work.
...
\paragraph{NMI Context.} Instrumentation is disabled in NMI handlers
(\texttt{in\_nmi()} check). Bugs triggered exclusively from NMI context would
be missed.
...
bitmap. We envision a composite feedback signal: unique
$(\mathit{PC}, \mathit{arg\_hash})$ pairs provide finer-grained state
discrimination than unique edges alone.

\paragraph{Upstream Path.} We plan to submit the LLVM pass as an RFC to
llvm-dev (SanitizerCoverage extension) and the kernel backend to
linux-kernel@vger.kernel.org (KCOV maintainer: Dmitry Vyukov).
...
KCOV~\cite{vyukov2016kcov} provides edge coverage---our foundation.
...
fuzzing and data-flow analysis for OS kernels. Our key insight is that function
...
The system requires no source annotation, operates safely in interrupt context,
...
We demonstrated correctness on five vulnerability classes across C and Rust
...
1 & \texttt{flags} & ENTRY=1, RET=2 \\
2 & \texttt{pc} & Instrumented function address \\
3 & \texttt{arg\_idx | arg\_size} & Packed: index (hi32) | size (lo32) \\
4 & \texttt{num\_fields} & Number of struct fields (0 for scalar) \\
5..5+N & \texttt{field\_val[0..N]} & Field values read via \texttt{copy\_from\_kernel\_nofault} \\
\hline
\end{tabular}
\end{center}
...
# 2. Instrument with our opt (uses custom LLVM 23)
