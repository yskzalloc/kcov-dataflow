...
obtain deterministic argument records for root-cause analysis without
...
\begin{tikzpicture}[
  ...
  layer/.style={rectangle, draw, thick, rounded corners, minimum width=6cm, minimum height=1cm, font=\footnotesize, align=center},
...
\node[font=\scriptsize\bfseries, anchor=north west] at (l1.north west) {\quad Compile Time};
\node[comp] at ([yshift=-0.1cm]l1.center) (pass) {LLVM SanCov Pass\\(dataflow-args/ret)};

% Layer 2: Kernel runtime
\node[layer, fill=orange!10, below=0.6cm of l1] (l2) {};
\node[font=\scriptsize\bfseries, anchor=north west] at (l2.north west) {\quad Kernel Runtime};
\node[comp, fill=orange!15] at ([xshift=-1.2cm,yshift=-0.1cm]l2.center) (cb) {\_\_sanitizer\_cov\\\_trace\_args/ret};
\node[comp, fill=orange!15] at ([xshift=1.5cm,yshift=-0.1cm]l2.center) (buf) {Per-Task\\Ring Buffer};

% Layer 3: User space
\node[layer, fill=green!10, below=0.6cm of l2] (l3) {};
\node[font=\scriptsize\bfseries, anchor=north west] at (l3.north west) {\quad User Space};
\node[comp, fill=green!15] at ([yshift=-0.1cm]l3.center) (user) {mmap() consumer\\(fuzzer / analyst)};

% Arrows
\draw[arrow] (pass) -- node[right, label] {inserts callbacks} (cb);
\draw[arrow] (cb) -- node[above, label] {atomic write} (buf);
\draw[arrow] (buf) -- node[right, label] {shared memory} (user);
\end{tikzpicture}
...
\node[pstep, right=0.6cm of rustc] (opt) {opt\\(our pass)};
...
incompatibility between rustc's bundled LLVM and our custom pass.}\label{fig:rust-pipeline}
\end{figure}
...
(which allows softirq tracing via \texttt{kcov\_remote\_start}); we chose
...
\hline
0 & \texttt{type\_and\_seq} & Packed: type marker (bits [31:28], \texttt{0xE} for entry, \texttt{0xF} for return) | seq (bits [23:0]) \\
1 & \texttt{pc} & Instrumented function address \\
2 & \texttt{meta} & Packed: arg\_idx (bits [63:56], 0 for return) | size (bits [55:48]) | pointer (bits [47:0]) \\
3..3+N & \texttt{field\_val[0..N]} & Field or scalar values read via \texttt{copy\_from\_kernel\_nofault} \\
\hline
\end{tabular}
\end{center}
